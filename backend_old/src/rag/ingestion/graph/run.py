...
def check_requirements():
    ...
    api_keys = project_root / "src" / "rag" / "api_key_manager" / "build_graph" / "api_key.txt"
    
    issues = []
    
    if not data_raw.exists():
        issues.append("[X] data/raw/ directory not found")
    
    if not input_json.exists():
        issues.append("[X] data/raw/input.json not found")
    else:
        print(f"   [OK] Found input file: {input_json}")
    
    if not api_keys.exists():
        issues.append("[X] src/rag/api_key_manager/build_graph/api_key.txt not found")
    else:
        # Count keys
        with open(api_keys, 'r') as f:
            key_count = len([line for line in f if line.strip()])
        print(f"   [OK] Found {key_count} API keys")
...
def get_user_choice():
    ...
    print("  3. Neo4j import only (CSV -> Neo4j)")
...
def get_config_for_extraction(include_neo4j=False, include_milvus=False):
    """Get configuration for extraction operations"""
    print("\n" + "-"*70)
    ...
    if mode == 'n':
        ...
        # Neo4j settings (if applicable)
        clear_neo4j = False
        if include_neo4j:
            clear_neo4j = input("\n  Clear existing Neo4j data? (y/N): ").strip().lower() == 'y'
        
        # Milvus settings (if applicable)
        ...
        clear_neo4j = False
        recreate_milvus = False
        print("  [OK] Using default settings")
...
def get_config_for_neo4j():
    """Get configuration for Neo4j import only"""
    print("\n" + "-"*70)
    print("NEO4J IMPORT CONFIGURATION")
    print("-"*70 + "\n")
    ...
    if clear_neo4j:
        ...
        print("  [i] Will append/update existing data")
...
def get_config_for_milvus():
    """Get configuration for Milvus embedding only"""
    print("\n" + "-"*70)
    print("MILVUS EMBEDDING CONFIGURATION")
    print("-"*70 + "\n")
    ...
    if recreate_milvus:
        ...
        print("  [i] Will append/update existing embeddings")
...
def check_status():
    ...
    if nodes_csv.exists():
        ...
        print(f"  [OK] Nodes extracted: {node_count}")
    else:
        print("  [!] No nodes.csv found")
    
    if edges_csv.exists():
        with open(edges_csv, 'r') as f:
            edge_count = sum(1 for _ in f) - 1
        print(f"  [OK] Edges extracted: {edge_count}")
    else:
        print("  [!] No edges.csv found")
    
    if processed_txt.exists():
        with open(processed_txt, 'r') as f:
            processed_count = len([line for line in f if line.strip()])
        print(f"  [OK] Processed items: {processed_count}")
    else:
        print("  [!] No processed_id.txt found")
...
def run_neo4j_only(pipeline, options):
    ...
    if options['clear_neo4j']:
        confirm = input("[!] This will DELETE all existing data in Neo4j. Continue? (yes/N): ")
...
def run_milvus_only(pipeline, options):
    ...
    if options['recreate_milvus']:
        confirm = input("[!] This will DELETE existing embeddings. Continue? (yes/N): ")
        if confirm.lower() != 'yes':
            print("[X] Cancelled")
            return False
...
def main():
    ...
    try:
        ...
        # Load API keys (only needed for extraction)
        api_keys = []
        if choice in ['1', '2']:
            api_key_file = project_root / "src" / "rag" / "api_key_manager" / "build_graph" / "api_key.txt"
        ...
        if choice in ['1', '2']:
            print(f"   [OK] Config: {options['model_name']}, batch={options['batch_size']}")
        ...
        if success:
            ...
            print("[X] OPERATION FAILED - Check logs for details")
        print("="*70 + "\n")
