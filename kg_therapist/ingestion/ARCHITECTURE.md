...
Module ingestion sử dụng **shared config và clients** từ `kg_therapist` để đảm bảo consistency across toàn bộ hệ thống.
...
**File**: `kg_therapist/vectors/embeddings.py`
...
**File**: `kg_therapist/vectors/milvus_client.py`

### Global Connection:
```python
from pymilvus import connections, Collection
from kg_therapist.config import MILVUS_URI, MILVUS_TOKEN, MILVUS_COLLECTION
...
**File**: `kg_therapist/graph/neo4j_client.py`

### Global Driver:
```python
from neo4j import GraphDatabase
from kg_therapist.config import NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD
...
**File**: `kg_therapist/ingestion/graph/llm_extractor.py`
...
**File**: `kg_therapist/ingestion/graph/milvus_embedder.py`
...
**File**: `kg_therapist/ingestion/graph/neo4j_writer.py`
...
**File**: `kg_therapist/ingestion/graph/pipeline.py`
...
pipeline = create_pipeline(
    api_keys=keys,
    # All config from kg_therapist.config by default
...
[Neo4j Writer] → Neo4j Graph Database
    ↑             ↓
    └─────────────┘
    (Uses kg_therapist.config)
    ↓
[Milvus Embedder] → Milvus Vector DB
    ↑
    └── Uses kg_therapist.vectors.embeddings (shared E5)
```

## 📦 Module Exports

**File**: `kg_therapist/ingestion/graph/__init__.py`
...
```python
from kg_therapist.ingestion.graph import create_pipeline, load_api_keys_from_file

# Load API keys
api_keys = load_api_keys_from_file('data/raw/api_key.txt')

# Create pipeline (uses kg_therapist.config)
...
```bash
# Uses all defaults from kg_therapist.config
...
Default values from `kg_therapist.config` are used if not specified.

## 📝 Notes

1. **E5 Model**: Singleton pattern, loaded once in `kg_therapist.vectors.embeddings`
2. **Milvus Connection**: Global connection in `kg_therapist.vectors.milvus_client`
3. **Neo4j Driver**: Global driver in `kg_therapist.graph.neo4j_client`
4. **Config**: Centralized in `kg_therapist.config` with env var support
