...
- ❌ **No empathy**: Crisis response có 2 versions không nhất quán
- ❌ **Jailbreak vulnerable**: Không chặn roleplay requests ("viết thư tuyệt mệnh")
- ❌ **Temporary disabled**: Safety check routing bị comment, luôn return "slot_filling"
- ⚠️ **Output format mismatch**: Logic function cần trả về format mới nhưng vẫn backward compatible
...
async def process_safety_check(
    ...
    summary_context: str = ""
) -> Dict[str, bool]:  # Currently returns {"is_high_risk": bool}
```

**⚠️ BREAKING CHANGES cần thiết:**
1. Thêm parameter `history: str` (formatted conversation history từ node wrapper)
2. Thay đổi output format từ `{"is_high_risk": bool}` → `{"parsed_output": {...}, "is_high_risk": bool}`
3. Remove query enhancement logic (không cần nữa vì có history riêng)
4. Parse new JSON schema từ prompt (classification, reason, confidence, indicators)
...
**⚠️ HIỆN TRẠNG**: File này hiện có **2 versions của crisis_response**:
- Version 1 (dòng 1-20): Hotline cũ (1800 599 199, 1800 599 994)
- Version 2 (dòng 22-50): Nội dung khác

**HÀNH ĐỘNG**: Thay thế toàn bộ crisis_response bằng version mới:
...
**⚠️ QUAN TRỌNG**: Giữ nguyên `not_mental_health_response` (không sửa).
crisis_response: |
  Tôi nghe được rằng bạn đang trải qua một giai đoạn rất khó khăn và đau khổ. Cảm giác này thật sự nặng nề, và tôi hiểu rằng có lúc mọi thứ có vẻ quá sức chịu đựng.
