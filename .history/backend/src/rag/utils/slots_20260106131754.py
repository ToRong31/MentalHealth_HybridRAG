...
def has_sufficient_slots(slots: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
    ...
    # Logic: Sufficient if:
    # 1. At least 4 out of 6 core REQUIRED slots filled (emotion, duration, trigger, impact, intensity, recent_life_events)
    # 2. Has some functional impairment info (not strictly required, but recommended)
    # 3. At most 1 differential slot missing (allows some flexibility)
    # 
    # Rationale: 
    # - 4/6 core slots = 67% coverage, enough to provide supportive guidance
    # - Functional impairment helps assess severity but not strictly blocking
    # - Can still gather more info in follow-up turns
    ...
    # Sufficient if: at least 4/6 core slots filled AND at most 1 differential missing
    is_sufficient = (core_filled_count >= 4) and (len(differential_missing) <= 1)
