...
def get_default_slots() -> Dict[str, Any]:
    """
    Return default empty slots structure aligned with DSM-5 diagnostic criteria.
    
    Slot Types:
    - Lists []: For accumulating multiple values (symptoms, triggers, etc.)
    - Tri-state strings: "yes"/"no"/"unknown" for gateway questions
    - Scalars: For single-value fields (confidence, certainty)
    - Derived fields (*_current): Computed from lists for severity comparison
    
    Note: Safety slots removed (handled by separate safety_check node)
    """
    return {
        # ===== PRESENTING =====
        "presenting_problem": [],      # Chief complaint
        "emotion": [],                 # Specific emotions
        "primary_mood": [],            # Underlying mood state
        ...
        "frequency": [],               # How often (daily, weekly, episodic)
        "symptom_fluctuation": [],     # Constant vs fluctuating
        "time_of_day_pattern": [],     # When symptoms worst (optional)

        # ===== SEVERITY =====
        "intensity": [],               # List of intensity descriptions
        "intensity_current": "unknown", # Derived: max severity from list
        "distress_level": [],          # Subjective distress (0-10 or mild/moderate/severe)
        "distress_level_current": "unknown", # Derived
        "stress_level": [],            # List of stress descriptions
        "stress_level_current": "unknown", # Derived
        "risk_level": [],              # Kept for backward compatibility

        # ===== FUNCTIONING/IMPAIRMENT =====
        "daily_functioning": [],
        "work_school_impact": [],
        "social_functioning": [],      # NEW: Impact on relationships/social
        "self_care_functioning": [],   # NEW: Impact on eating, hygiene, self-care
        "impact": [],                  # Kept for backward compatibility
        ...
        # ===== SUPPORT/COPING =====
        "support_system": [],
        "family_support": [],          # Kept for backward compatibility
        "friend_support": [],          # Kept for backward compatibility
        "social_isolation": None,      # Boolean (backward compat)
        "social_withdrawal": None,     # Boolean (backward compat)
        "coping_mechanisms": [],
        "coping_effectiveness": [],
        "need": [],

        # ===== EXCLUSION (CRITICAL for differential diagnosis) =====
        "substance_use_any": "unknown",    # NEW: Gateway tri-state
        "substance_use": [],               # Details if any=yes
        "medical_history_any": "unknown",  # NEW: Gateway tri-state
        "medical_history": [],             # Details if any=yes
        "medication_changes": [],          # NEW: Recent med changes
        "caffeine_nicotine_use": [],       # NEW: Stimulant use

        # ===== TREATMENT (current) =====
        "current_treatment": [],
        "medication": [],
        "history_of_trauma": [],       # Kept for context

        # ===== CROSS-CUTTING SCREENS =====
        "mania_like_symptoms": "unknown",    # NEW: Screen for bipolar
        "psychotic_like_symptoms": "unknown", # NEW: Screen for psychosis

        # ===== DERIVED/META =====
        "diagnostic_confidence": "low",
        "potential_differentials": [],
        "duration_certainty": "vague",
        "intake_complete": False       # NEW: All required slots filled
    }
...
def merge_slots(existing_slots: Optional[Dict[str, Any]], new_slots: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, new_value in new_slots.items():
        ...
        # CRITICAL FIX: Only skip truly empty values
        # DO NOT skip "no", "none" strings - these are valid answers!
        ...
        if isinstance(new_value, list):
            if not existing_value:
                ...
                # Convert to lowercase for comparison but keep original
                seen = set()
                unique = []
                ...
                seen = set(item.lower() if isinstance(item, str) else item for item in existing_value)
                for item in new_value:
                    if item is None or (isinstance(item, str) and item.strip() == ""):
                        continue
                    item_lower = item.lower() if isinstance(item, str) else item
            ...
            # CRITICAL FIX: Derive *_current field for severity lists
            if key in SEVERITY_ORDER:
                current_key = key + "_current"
                merged[current_key] = max_severity(key, merged[key])
                logger.debug(f"Derived {current_key} = {merged[current_key]} from {merged[key]}")
        
        # Handle tri-state fields (yes/no/unknown)
        elif key.endswith("_any") or key in ["mania_like_symptoms", "psychotic_like_symptoms"]:
            # Update if new value is more informative than existing
