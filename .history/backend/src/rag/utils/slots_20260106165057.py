...
def has_sufficient_slots(slots: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
    ...
    # Check functional impairment (IMPORTANT for severity assessment, but not blocking)
    ...
    if not has_functional_info:
        # Add to differential (recommended) rather than required (blocking)
    ...
    # Check differential diagnosis slots (IMPORTANT for preventing misdiagnosis)
    # Medical exclusion: Less strict, ask but don't block
    physical_symptoms = slots.get("physical_symptoms", [])
    
    # If physical symptoms present → SHOULD have medical_history AND substance_use (but not blocking)
    if physical_symptoms and len(physical_symptoms) > 0:
        if not slots.get("medical_history"):
            differential_missing.append("medical_history")  # Recommended, not required
        if not slots.get("substance_use"):
            differential_missing.append("substance_use")  # Recommended, not required
    else:
        # No physical symptoms → still check but optional
        if not slots.get("medical_history"):
    ...
    # Logic: Sufficient if:
    # 1. ALL core REQUIRED slots filled (emotion, duration, trigger, impact, intensity, recent_life_events)
    # 2. At most 2 differential slots missing (flexible for medical history, substance use)
    # 
    # Rationale:
    # - Need complete picture for accurate assessment and diagnosis
    # - Prevents premature conclusions with incomplete information
    # - Ensures systematic information gathering
    # - Differential slots (medical/substance) are recommended but not blocking
    
    # Count filled core REQUIRED slots
    # required_missing đã loại trừ specific_duration (vì là derived field)
    # Nên chỉ cần đếm các slot core còn thiếu
    core_required_count = len(REQUIRED_SLOTS)
    core_missing_count = len([m for m in required_missing if m not in ['specific_duration']])
    core_filled_count = core_required_count - core_missing_count
    
    # STRICT: ALL core REQUIRED slots phải được điền (6/6)
    # Chỉ cho phép thiếu specific_duration (duration chất lượng kém)
    # Differential slots (functional, medical, substance) không blocking
    is_sufficient = (core_missing_count == 0)
    
    logger.info(f"[SUFFICIENCY CHECK] Core filled: {core_filled_count}/{core_required_count}, Required missing: {required_missing}, Differential missing: {len(differential_missing)}, Result: {is_sufficient}")
    
    # Debug: log chi tiết từng slot
    logger.debug(f"[SLOT DETAILS] Filled slots: {[k for k in REQUIRED_SLOTS if slots.get(k) not in [None, [], 'none', '']]}, Missing slots: {required_missing}")
