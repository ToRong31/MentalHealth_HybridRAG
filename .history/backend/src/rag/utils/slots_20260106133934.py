...
def has_sufficient_slots(slots: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
    ...
    # Count filled core REQUIRED slots (excluding functional_impairment, medical_history, substance_use which are differential)
    core_required_count = len(REQUIRED_SLOTS)
    core_filled_count = core_required_count - len([m for m in required_missing if m not in ['functional_impairment', 'medical_history', 'substance_use', 'specific_duration']])
    
    # STRICT: ALL 6/6 core REQUIRED slots must be filled
    # Differential slots (functional, medical, substance) can be missing (up to 2)
    is_sufficient = (core_filled_count >= core_required_count) and (len(differential_missing) <= 2)
    
    logger.info(f"[SUFFICIENCY CHECK] Core filled: {core_filled_count}/{core_required_count}, Required missing: {[m for m in required_missing if m not in ['functional_impairment', 'medical_history', 'substance_use']]}, Differential missing: {len(differential_missing)}, Result: {is_sufficient}")
