...
def has_sufficient_slots(slots: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
    ...
    # Logic: Sufficient if:
    # 1. At least 2 out of 6 core REQUIRED slots filled (LOWERED threshold for better UX)
    #    - Rationale: 2 slots (e.g., emotion + duration) enough to start providing guidance
    #    - Can gather more details progressively in conversation
    # 2. At most 2 differential slots missing (flexible)
    # 
    # Why 2/6 instead of 4/6:
    # - User experience: Don't block with too many questions upfront
    # - Progressive disclosure: Gather info naturally through conversation
    # - Even basic info (emotion + duration) enables supportive response
    # - Can still ask follow-up questions AFTER providing initial guidance
    
    # Count filled core REQUIRED slots (excluding functional_impairment which is checked separately)
    ...
    # LOWERED threshold: 2/6 = 33% (was 4/6 = 67%)
    # This allows proceeding with minimal but meaningful info
    is_sufficient = (core_filled_count >= 2) and (len(differential_missing) <= 2)
    
    logger.debug(f"[SUFFICIENCY CHECK] Core filled: {core_filled_count}/{core_required_count}, Differential missing: {len(differential_missing)}, Result: {is_sufficient}")
