...
# Risk assessment conditional required slots (only ask if high-risk indicators detected)
...
def get_default_slots() -> Dict[str, Any]:
    ...
    return {
        ...
        # K. RISK ASSESSMENT (conditional - only ask if high-risk indicators)
        "self_harm_risk": "unknown",         # TRI-STATE: yes/no/unknown
        "suicide_risk": "unknown",           # TRI-STATE: yes/no/unknown
        "safety_plan": [],                   # Safety net, support available
...
def next_missing_slot(slots: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
    ...
    PRIORITY: If high-risk indicators detected, prioritize risk assessment before continuing.
    ...
    """
    # === PRIORITY 1: Check for high-risk indicators ===
    is_high_risk, risk_level, risk_indicators = detect_high_risk_indicators(slots)
