...
class BaseRetrieval(ABC):
    ...
    async def retrieve_async(self, query: str, **kwargs) -> 'RetrievalResult':
        ...
        Now directly calls async retrieve (no executor needed).
        ...
        """
        # If retrieve is async, call it directly
        import inspect
        if inspect.iscoroutinefunction(self.retrieve):
            return await self.retrieve(query, **kwargs)
        
        # Fallback for sync retrieve (run in executor)
