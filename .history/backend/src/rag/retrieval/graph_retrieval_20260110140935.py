...
class GraphRetrieval(BaseRetrieval):
    ...
    async def retrieve(self, query: str, **kwargs) -> RetrievalResult:
        """
        Retrieve graph context for query (ASYNC).
        ...
        # Step 4: Rerank anchors with Cohere (use original query for reranking) - ASYNC
        # Original query ensures reranking focuses on user's actual intent
        rerank_top_k = kwargs.get('rerank_top_k', self.rerank_top_k)
        anchors = await reranker.rerank_anchors(original_query, candidates, top_k=rerank_top_k)
