...
async def load_prompts_async(filename: str = None) -> Union[Dict[str, Any], Dict[str, str]]:
    """
    Load prompts from YAML files asynchronously (non-blocking)
    ...
    therapist_file = prompts_dir / "therapist_prompt.yaml"
    if therapist_file.exists():
        async with aiofiles.open(therapist_file, 'r', encoding='utf-8') as f:
            content = await f.read()
            data = yaml.safe_load(content)
            prompts['therapist_prompt'] = data.get('prompt', '')
    ...
    safety_file = prompts_dir / "safety_check_prompt.yaml"
    if safety_file.exists():
        async with aiofiles.open(safety_file, 'r', encoding='utf-8') as f:
            content = await f.read()
            data = yaml.safe_load(content)
            prompts['safety_check_prompt'] = data.get('prompt', '')
            prompts['crisis_response'] = data.get('crisis_response', '')
