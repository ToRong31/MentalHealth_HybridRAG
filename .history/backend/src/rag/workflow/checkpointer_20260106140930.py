...
class CheckpointerManager:
    ...
    def _initialize_checkpointer(self):
        ...
        try:
            ...
            # CRITICAL: AsyncPostgresSaver.setup() is an async coroutine!
            # We need to call it in an async context, but _initialize_checkpointer is sync.
            # Solution: Use asyncio.run() to run the coroutine in a new event loop.
            import asyncio
            try:
                # Try to get the current event loop
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    # If we're already in an async context, this will be handled by caller
                    # For now, just skip setup and log warning
                    logger.warning("⚠️ Skipping setup() in sync context - must be called async!")
                    logger.warning("⚠️ Call await get_checkpointer().setup() from async context!")
                else:
                    # Run setup in new event loop
                    asyncio.run(self._checkpointer.setup())
            except RuntimeError:
