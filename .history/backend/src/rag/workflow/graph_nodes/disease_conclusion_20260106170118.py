...
async def verify_diseases_with_context(
    ...
    detected_disease, confidence, reasoning = await analyze_disease(
        diagnostic_chunks,
        rewritten_query,
        slots,
        conversation_buffer,
...
async def disease_conclusion_node(state: KGState) -> KGState:
    ...
    try:
        ...
        # verify_diseases_with_context is now async, so call it directly
        detected_disease, confidence, reasoning = await verify_diseases_with_context(
            diagnostic_diseases,
            diagnostic_chunks,
            rewritten_query,
            slots,
