...
async def slot_filling_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(f"[SLOT FILLING] Calling LLM to extract slots from question...")
    result = await process_slot_filling(enhanced_question, existing_slots=existing_slots)
    new_slots = result.get("slots", {})
    
    logger.info(f"[SLOT FILLING] LLM extracted {len([k for k, v in new_slots.items() if v and v != [] and v != 'none'])} slots")
    logger.debug(f"[SLOT FILLING] Extracted slots: {[(k, v) for k, v in new_slots.items() if v and v != [] and v != 'none']}")
    ...
    # CRITICAL: Filter out any slots that are already filled in merged_slots
    # This prevents asking about information user already provided
    actually_missing = []
    for slot in relevant_missing_slots:
        slot_value = merged_slots.get(slot)
        if slot_value is None or slot_value == [] or slot_value == 'none' or slot_value == '':
            actually_missing.append(slot)
        else:
            logger.info(f"[FILTER] Removing '{slot}' from missing list - already filled with: {slot_value}")
    
    relevant_missing_slots = actually_missing
    logger.info(f"[FILTER] After filtering: {len(actually_missing)} slots actually missing from {len(result.get('relevant_missing_slots', []))} originally reported")
