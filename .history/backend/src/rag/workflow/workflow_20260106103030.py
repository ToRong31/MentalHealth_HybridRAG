...
def route_after_slot_filling(state: KGState) -> Literal["query_rewriter", "request_more_info"]:
    ...
    - Nếu đủ slots -> query_rewriter (directly to diagnostic flow)
...
def build_kg_graph():
    ...
    # Conditional routing after slot filling (SIMPLIFIED)
    builder.add_conditional_edges(
        "slot_filling",
        route_after_slot_filling,
        {
            "query_rewriter": "query_rewriter",  # Đủ slots -> query_rewriter -> diagnostic
