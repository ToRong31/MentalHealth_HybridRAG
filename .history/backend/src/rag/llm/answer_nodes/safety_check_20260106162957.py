...
# Safety check prompt will be loaded lazily in the async function
safety_check_prompt = None
...
async def process_safety_check(
    ...
    # Lazy load prompt if not loaded yet
    global safety_check_prompt
    if safety_check_prompt is None:
        safety_check_prompt_data = await load_prompts("safety_check_prompt.yaml")
