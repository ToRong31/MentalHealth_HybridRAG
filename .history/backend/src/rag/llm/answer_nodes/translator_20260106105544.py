...
import aiofiles  # nhớ: pip install aiofiles
...
class GeminiTranslator(LLMClient):
    ...
    def __init__(
        ...
        # Load prompts asynchronously (safe for initialization)
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                # If event loop is already running, schedule coroutine
                asyncio.create_task(self._load_prompts(prompt_config_path))
            else:
                # If no event loop running, run synchronously
                loop.run_until_complete(self._load_prompts(prompt_config_path))
        except RuntimeError:
            # No event loop, create new one
            asyncio.run(self._load_prompts(prompt_config_path))

    # ------------------- PROMPT LOADING ------------------- #

    async def _load_prompts(self, path: str):
        """Load prompts from YAML file asynchronously"""
