...
class GeminiTranslator(LLMClient):
    ...
    def __init__(
        ...
        # Load prompts using asyncio (run in thread to avoid blocking)
        asyncio.run(self._load_prompts_async(prompt_config_path))

    # ------------------- PROMPT LOADING ------------------- #

    async def _load_prompts_async(self, path: str):
        """Load prompts asynchronously to avoid blocking"""
    ...
    def _load_prompts(self, path: str):
        ...
        asyncio.run(self._load_prompts_async(path))
