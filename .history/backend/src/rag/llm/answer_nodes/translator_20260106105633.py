...
# Module-level prompt cache - loaded once at import time (before ASGI server starts)
# This avoids blocking I/O during request handling
_PROMPT_CACHE: Dict[str, Dict[str, str]] = {}


def _load_prompts_sync(path: str) -> Dict[str, str]:
    """
    Load prompts from YAML file synchronously at module import time.
    ...
    prompts = {
        'question': data.get("question_prompt", "").strip(),
        'answer': data.get("answer_prompt", "").strip()
    }
    
    if not prompts['question']:
        raise ValueError("question_prompt is missing or empty in YAML config")
    if not prompts['answer']:
        raise ValueError("answer_prompt is missing or empty in YAML config")
    
    _PROMPT_CACHE[path] = prompts
    return prompts
...
class GeminiTranslator(LLMClient):
    ...
    Prompts được load từ YAML và cache ở module level.
    """
    ...
    def __init__(
        ...
        # Get from module-level cache (loaded at import time, not during request)
        prompts = _load_prompts_sync(prompt_config_path)
        self._question_prompt_template = prompts['question']
        self._answer_prompt_template = prompts['answer']
