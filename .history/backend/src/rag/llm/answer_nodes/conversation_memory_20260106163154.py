...
# Summarization prompts will be loaded lazily
summarize_pair_prompt_template = None
summarize_buffer_prompt_template = None
...
async def summarize_pair(pair: Dict[str, str]) -> str:
    ...
    Returns:
        Summary string
    """
    # Lazy load prompt templates if not loaded yet
    global summarize_pair_prompt_template, summarize_buffer_prompt_template
    if summarize_pair_prompt_template is None:
        try:
            summarize_prompt_data = await load_prompts("conversation_summarize_prompt.yaml")
