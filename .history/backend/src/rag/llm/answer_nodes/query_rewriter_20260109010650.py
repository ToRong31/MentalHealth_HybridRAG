...
try:
    ...
    # Log first 200 chars to verify prompt content
    logger.info(f"✅ Successfully loaded query rewrite prompt from YAML (length: {len(QUERY_REWRITE_TEMPLATE)} chars)")
    logger.info(f"📝 Prompt preview: {QUERY_REWRITE_TEMPLATE[:200]}...")
...
async def rewrite_query_with_slots(
    ...
    try:
        ...
        # ⚠️ IMPORTANT: Do NOT inject assessment/screening context into query
        # Assessment context is for disease_conclusion, NOT for retrieval
        # This ensures retrieval is based purely on symptoms, not pre-judgments
