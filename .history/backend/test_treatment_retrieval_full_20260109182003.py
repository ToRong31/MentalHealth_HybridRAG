...
def test_field_names():
    ...
    results = col.query(
        expr="",
        limit=2,
        output_fields=["node_id", "disease"]  # Field is "disease" not "chunk_id"
    )
    
    print(f"\n✅ Sample records:")
    for i, record in enumerate(results, 1):
        print(f"   {i}. node_id={record.get('node_id')}, disease='{record.get('disease')[:50]}...'")
...
async def test_milvus_query(mapping):
    ...
    expr = f'disease in [{title_list_str}]'  # Field is "disease"
    ...
    results = col.search(
        ...
        limit=5,
        expr=expr,
        output_fields=["node_id", "disease"]  # Field is "disease"
    )
    ...
    for i, hit in enumerate(hits, 1):
        ...
        print(f"      disease='{hit.entity.get('disease')}'")
