...
export function ChatSidebar({ 
  ...
  const formatTime = (dateString?: string) => {
    ...
    // Parse date - ALWAYS parse as UTC to match server timestamps
    let date: Date;
    if (typeof dateString === 'string') {
      // If no timezone info, assume UTC (server sends UTC)
    ...
    if (isNaN(date.getTime())) {
      console.warn('[ChatSidebar] Invalid date:', dateString);
      return '';
    }
    
    // Debug log - tất cả so sánh ở UTC
    const now = new Date();
    const diffHours = Math.floor((now.getTime() - date.getTime()) / (1000 * 60 * 60));
    console.log('[ChatSidebar] Formatting time:', {
      input: dateString,
    ...
    const diffHours2 = Math.floor(diffMins / 60);
    const diffDays = Math.floor(diffHours2 / 24);
    ...
    // Dưới 24 giờ
    if (diffHours2 < 24) return `${diffHours2} giờ trước`;
    // Hôm qua
