"""
Support Agent — Standalone FastAPI microservice.
"""
...
def _build_llm():
    """Build LLM — prefers NVIDIA API, falls back to Gemini."""
    if api_key := os.getenv("NVIDIA_API_KEY"):
        try:
            ...
            logger.warning("[Support] OpenAI LLM init failed: %s", e)
    ...
    if api_key := os.getenv("GEMINI_API_KEY"):
        try:
            ...
            logger.warning("[Support] Gemini init failed: %s", e)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning("[Support] Milvus unavailable: %s", e)
    ...
    if pwd := os.getenv("NEO4J_PASSWORD"):
        try:
            ...
            logger.warning("[Support] Neo4j unavailable: %s", e)
    ...
    if api_key := os.getenv("COHERE_API_KEY"):
        try:
            ...
            logger.warning("[Support] Reranker unavailable: %s", e)
    ...
    logger.info("[Support] Started on port %s", os.getenv("AGENT_PORT", "8104"))
    yield
    logger.info("[Support] Shutdown complete")


app = FastAPI(title="Support Agent", version="1.0.0", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])


@app.get("/health", response_model=HealthResponse)
async def health():
    return HealthResponse(status="ok", agent="support", port=int(os.getenv("AGENT_PORT", "8104")))


@app.post("/agent/support", response_model=AgentResponse)
async def run_support(req: AgentRequest):
    ...
    try:
        ...
        logger.error("[Support] error: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))


if __name__ == "__main__":
    port = int(os.getenv("AGENT_PORT", "8104"))
    uvicorn.run("ai.agents.support.main:app", host="0.0.0.0", port=port, reload=False)
