"""
Theory Agent — Standalone FastAPI microservice.
"""
...
def _build_llm():
    if api_key := os.getenv("NVIDIA_API_KEY"):
        try:
            ...
            logger.warning("[Theory] OpenAI LLM init failed: %s", e)
    ...
    if api_key := os.getenv("GEMINI_API_KEY"):
        try:
            ...
            logger.warning("[Theory] Gemini init failed: %s", e)
...
async def health():
    return HealthResponse(
        status="ok", agent="theory", port=int(os.getenv("AGENT_PORT", "8102"))
    )


@app.post("/agent/theory", response_model=AgentResponse)
async def run_theory(req: AgentRequest):
    ...
    try:
        ...
        logger.error("[Theory] error: %s", e, exc_info=True)
