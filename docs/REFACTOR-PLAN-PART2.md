...
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────┼──────────────────────────────┐
│           LOCAL MEMORY (Agent-Private, Ephemeral)    │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐     │
│  │ ReAct loop │  │ Intermediate│ │ Agent-local │     │
│  │ state:     │  │ tool results│ │ cache:      │     │
│  │ step, iter │  │ retrieval   │ │ embedding,  │     │
│  │ count      │  │ candidates  │ │ rerank      │     │
│  └────────────┘  └────────────┘  └────────────┘     │
│  Persistence: In-memory (cleared at end of turn)   │
│  Access: Only owner agent                           │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────┼──────────────────────────────┐
│         SHARED MEMORY (All agents, Persistent)       │
│  ┌──────────────────────────────────────────────┐   │
│  │ { buffer: [...3 Q&A pairs],                   │   │
│  │    summary: "...older turns...",               │   │
│  │    slots: {emotion, trigger, duration...},    │   │
│  │    crisis_state: {...} }                       │   │
│  └──────────────────────────────────────────────┘   │
│  Persistence: PostgreSQL (L3) + Redis (L2)          │
│  Access: Any agent via MemoryService (injected)     │
│  ⚠️ Cleared when conversation ends                  │
└──────────────────────────────────────────────────────┘
```
...
```python
# ai/modules/shared/communication/message_bus.py
...
STEP 3: (Sau khi đủ slots)
  ...
  │            TheoryAgent → ConceptRetrieval Skill → rag/retrieval/ → Milvus + Neo4j + Cohere
  │
