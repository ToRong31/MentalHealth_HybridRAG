...
async def slot_filling_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Slots are extracted from the full conversation context (current question + buffer + summary).
    ...
    Returns:
        Updated state with slots, has_sufficient_slots, 
    ...
    # Call logic function with enhanced context
    result = await process_slot_filling(enhanced_question)
    
    # Check if REQUIRED slots are sufficient for retrieval
    slots = result.get("slots", {})
    is_sufficient, required_missing, _ = has_sufficient_slots(slots)
    
    logger.info(f"[DEBUG] is_sufficient = {is_sufficient}, required_missing = {required_missing}")
    logger.info(f"[DEBUG] Filled REQUIRED slots: {[s for s in ['emotion', 'primary_mood', 'intensity', 'trigger', 'duration', 'impact', 'need', 'stress_level'] if slots.get(s) not in [None, [], 'none']]}")
