...
class HybridRetriever(BaseRetriever):
    """
    Hybrid retriever that combines dense and graph retrieval.
    Executes both retrievers concurrently using asyncio.gather() for better performance.
    """
    ...
    def __init__(
        ...
        """
        self.graph_retriever = graph_retriever or graph_retrieval
        self.dense_retriever = dense_retriever or dense_retrieval
    ...
    async def retrieve_async(self, query: str, **kwargs) -> RetrievalResult:
        ...
        try:
            ...
            graph_task = loop.run_in_executor(
                _executor,
                lambda: self.graph_retriever.retrieve(query, **graph_kwargs)
            )
            dense_task = loop.run_in_executor(
                _executor,
                lambda: self.dense_retriever.retrieve(query, **dense_kwargs)
            )
